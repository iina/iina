...
class UPnPManager {
  ...
  private func sendMSearch() {
    ...
    if let multicast = try? NWMulticastGroup(for: [multicastEndpoint]) {
  ...
  private func parseSSDPResponse(_ response: String) {
    ...
    guard let st = headers["ST"] ?? headers["NT"],
          st.contains("MediaServer") else {
      return
    }
  ...
  private func parseDeviceDescription(xml: String, baseDevice: UPnPDevice) -> UPnPDevice? {
    ...
    let servicePattern = #"<service>[\s\S]*?<serviceType>(.*?)</serviceType>[\s\S]*?<serviceId>(.*?)</serviceId>[\s\S]*?<controlURL>(.*?)</controlURL>"#
  ...
  private func parseDIDLLite(xml: String, baseURL: URL) throws -> [UPnPItem] {
    ...
    let itemPattern = #"<(item|container)[\s\S]*?id="([^"]+)"[\s\S]*?parentID="([^"]+)"[\s\S]*?>(.*?)</(item|container)>"#
