...
class KeyMapping: NSObject {
  ...
  static func generateInputConf(from mappings: [KeyMapping]) -> String {
    // Fix issue #4217: Filter out default-bindings start to prevent mpv from loading default bindings
    // which would cause duplicate key bindings and double execution of seek commands
    let filteredMappings = mappings.filter { mapping in
      !(mapping.rawKey == "default-bindings" && mapping.action.count == 1 && mapping.action[0] == "start")
    }
    return filteredMappings.reduce("# Generated by IINA\n\n", { prevLines, km in prevLines + "\(km.confFileFormat)\n" })
  }
}
