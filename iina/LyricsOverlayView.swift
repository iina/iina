...
final class LyricsOverlayView: NSView {
  ...
  private func setupView() {
    ...
    private func setupLabels() {
        ...
        // Current line (focus)
        currentLabel.font = NSFont.systemFont(ofSize: 15, weight: .semibold)
    ...
    func update(state: LyricsOverlayState, animated: Bool = true) {
        ...
        if currentState?.current?.time == state.current?.time {
            return
        }
    ...
    func clear(animated: Bool = true) {
        let empty = LyricsOverlayState(previous: nil, current: nil, next: nil)
        update(state: empty, animated: animated)
    }
}
#if DEBUG
